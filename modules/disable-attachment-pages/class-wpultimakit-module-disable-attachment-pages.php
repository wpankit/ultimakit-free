<?php
/**
 * Class UltimaKit_Module_Disable_Attachment_Pages
 *
 * @since 1.0.0
 * @package    UltimaKit
 */
 ...
 * @since 1.0.0
 */
class UltimaKit_Module_Disable_Attachment_Pages extends UltimaKit_Module_Manager {
	/**
	 *
	 * @var string
	 */
	protected $ID = 'ultimakit_disable_attachment_pages';
	 ...
	 * @var string
	 */
	protected $read_more_link = 'disable-dynamic-attachement-pages-in-wordpress';
	...
	public function __construct() {
		$this->name        = __( 'Disable Attachment Pages', 'ultimakit-for-wp' );
		$this->description = __( 'Disable Attachment Pages: Redirects attachment pages to their parent post or the homepage, improving user experience and SEO.', 'ultimakit-for-wp' );
	...
	protected function initializeModule() {
		if ( $this->is_active ) {
			...
			add_action( 'template_redirect', array( $this, 'ultimakit_disable_attachment_pages' ) );
	...
	public function add_modal() {
		...
		$arguments['title'] = __( 'Disable Attachment Pages', 'ultimakit-for-wp' );

		$arguments['fields'] = array(
			'redirect_url' => array(
				'type'        => 'text',
				'label'       => __( 'Redirect URL', 'ultimakit-for-wp' ),
				'value'       => $this->getModuleSettings( $this->ID, 'redirect_url' ),
	...
	// Function to redirect attachment pages.
	public function ultimakit_disable_attachment_pages() {
		if ( is_attachment() ) {
			global $post;

			$redirect_url = $this->getModuleSettings( $this->ID, 'redirect_url' );
