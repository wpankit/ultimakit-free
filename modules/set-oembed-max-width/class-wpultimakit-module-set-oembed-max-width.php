<?php
/**
 * Class UltimaKit_Module_Set_Oembed_Max_Width
 *
 * @since 1.0.0
 * @package    UltimaKit
 */
 ...
 * @since 1.0.0
 */
class UltimaKit_Module_Set_Oembed_Max_Width extends UltimaKit_Module_Manager {
	/**
	 *
	 * @var string
	 */
	protected $ID = 'ultimakit_set_oembed_max_width';
	 ...
	 * @var string
	 */
	protected $read_more_link = 'set-oembed-max-width-in-wordpress';
	...
	public function __construct() {
		$this->name        = __( 'Set oEmbed Max Width', 'ultimakit-for-wp' );
		$this->description = __( 'Set a max width for the embeds using oEmbed in the content.', 'ultimakit-for-wp' );
	...
	protected function initializeModule() {
		if ( $this->is_active ) {
			...
			add_filter( 'embed_defaults', array( $this, 'ultimakit_oembed_defaults' ) );
		}
	}
	...
	public function add_modal() {
		...
		$arguments['title'] = __( 'Set oEmbed Max Width', 'ultimakit-for-wp' );

		$arguments['fields'] = array(
			'width'  => array(
				'type'  => 'number',
				'label' => __( 'Width', 'ultimakit-for-wp' ),
				'value' => $this->getModuleSettings( $this->ID, 'width', 400 ),
				'desc'  => __( 'Default: <strong>400px</strong>', 'ultimakit-for-wp' ),
			),
			'height' => array(
				'type'  => 'number',
				'label' => __( 'Height', 'ultimakit-for-wp' ),
				'value' => $this->getModuleSettings( $this->ID, 'height', 280 ),
				'desc'  => __( 'Default: <strong>280px</strong>', 'ultimakit-for-wp' ),
			),
		);
	...
	public function ultimakit_oembed_defaults( $sizes ) {
		return array(
			'width'  => $this->getModuleSettings( $this->ID, 'width' ),
			'height' => $this->getModuleSettings( $this->ID, 'height' ),
		);
	}
}
