...
class UltimaKit_Module_Schema_Markup_Generator extends UltimaKit_Module_Manager {
	...
	protected function initializeModule() {
		...
		if ( $this->is_active ) {
            ...
            // AJAX handlers
            add_action('wp_ajax_validate_schema', [$this, 'ajax_validate_schema']);
    ...
    public function add_meta_box() {
        $post_types = apply_filters('ultimakit_schema_post_types', ['post', 'page', 'product']);
    ...
    public function render_meta_box($post) {
        ...
        $saved_data = !empty($schema_data) ? json_decode($schema_data, true) : [];
        ?>
									...
									if (!empty($opening_hours) && is_array($opening_hours)) {
										foreach ($opening_hours as $hours_string) {
											...
											if (count($parts) === 2) {
												...
												if ($full_day) {
													$parsed_hours[strtolower($full_day)] = [
														'from' => $times[0],
														'to' => $times[1]
													];
												}
   ...
   /**
		...
		switch ($schema_type) {
			case 'FAQPage':  // Changed from 'FAQ' to 'FAQPage'
				if (isset($_POST['faq_schema'])) {
					$questions = isset($_POST['faq_schema']['questions']) ? $_POST['faq_schema']['questions'] : [];
					$answers = isset($_POST['faq_schema']['answers']) ? $_POST['faq_schema']['answers'] : [];
	...
	public function ajax_validate_schema() {
		...
		if ($json_error !== JSON_ERROR_NONE) {
			$error_message = sprintf(
				__('JSON Error (%d): %s. Raw data: %s','ultimakit-for-wp'),
				$json_error,
				$json_error_msg,
	...
	private function validate_article_schema($schema) {
		...
		$validArticleTypes = [
			__('Article','ultimakit-for-wp'), __('NewsArticle','ultimakit-for-wp'), __('BlogPosting','ultimakit-for-wp'), __('TechArticle','ultimakit-for-wp'), 
			__('ScholarlyArticle','ultimakit-for-wp'), __('SocialMediaPosting','ultimakit-for-wp')
		];
		...
		if (isset($schema->author)) {
			...
			if (!isset($schema->author->{'@type'}) || 
				!in_array($schema->author->{'@type'}, ['Person', 'Organization'])) {
				return [
					'valid' => false,
		...
		if (isset($schema->mainEntityOfPage)) {
			if (is_string($schema->mainEntityOfPage)) {
				...
				if (!isset($schema->mainEntityOfPage->{'@type'}) || 
					!in_array($schema->mainEntityOfPage->{'@type'}, ['WebPage', 'URL'])) {
					return [
						'valid' => false,
    ...
    private function validate_review_schema($schema) {
        $required_fields = ['itemReviewed', 'author', 'reviewRating', 'reviewBody'];
    ...
    private function enqueue_scripts() {
        ...
        wp_localize_script('ultimakit-schema-generator', 'ultimakitSchema', [
	...
	private function validate_local_business_schema($schema) {
		...
		$validBusinessTypes = [
			'LocalBusiness', 'Store', 'Restaurant', 'Hotel', 'Service',
			'MedicalBusiness', 'AutomotiveBusiness', 'FinancialService',
			'SportsActivityLocation', 'EducationalOrganization', 'ProfessionalService'
		];
