...
class UltimaKit_Module_Activity_Logs extends UltimaKit_Module_Manager {
    ...
    protected function initializeModule() {
        if ($this->is_active) {
            ...
            add_action('admin_logger_cleanup_cron', [$this, 'delete_old_logs']);
            add_action('wp', [$this, 'schedule_retention_cleanup']);
	...
	public function add_scripts() {
        ...
        wp_localize_script('ultimakit-module-script-' . $this->ID, 'ultimakitL10n', array(
    ...
    private function detect_profile_changes($user_id) {
        ...
        foreach ($fields_to_check as $field => $label) {
            ...
            $meta_fields = array('phone', 'address', 'city', 'state', 'country', 'zip', 
                            'company', 'position', 'expertise', 'facebook', 'twitter', 
                            'linkedin', 'instagram', 'email_notifications', 
    ...
    public function log_post_status_change($new_status, $old_status, $post) {
        ...
        try {
            ...
            $is_scheduled = !empty($post->post_date_gmt) && 
                        strtotime($post->post_date_gmt) > current_time('timestamp', true);
    ...
    public function log_category_update($term_id) {
        ...
        if ($old_term_data) {
            if ($old_term_data['name'] !== $term->name) {
                $changes['name'] = array('from' => $old_term_data['name'], 'to' => $term->name);
            }
            if ($old_term_data['slug'] !== $term->slug) {
                $changes['slug'] = array('from' => $old_term_data['slug'], 'to' => $term->slug);
            }
            if ($old_term_data['parent'] !== $term->parent) {
                $changes['parent'] = array('from' => $old_term_data['parent'], 'to' => $term->parent);
            }
            if ($old_term_data['description'] !== $term->description) {
                $changes['description'] = array('from' => $old_term_data['description'], 'to' => $term->description);
            }
        }
    ...
    public function handle_csv_export() {
        ...
        // Add CSV headers
        fputcsv($output, array_map(array($this, 'escape_csv_cell'), array(
