...
class UltimaKit_Module_Redirect_Manager extends UltimaKit_Module_Manager {
    ...
    public function enqueue_admin_assets($hook) {
        ...
        wp_localize_script('wpuk-redirect-manager', 'wpukRedirect', array(
    ...
    public function ajax_save_redirect() {
        ...
        if (empty($source_url) || empty($target_url)) {
            wp_send_json_error(['message' => __('Source URL and Target URL are required', 'ultimakit-for-wp')]);
            return;
        }
        ...
        if ($exists) {
            wp_send_json_error(['message' => __('A redirect with this source URL already exists', 'ultimakit-for-wp')]);
            return;
        }
        ...
        if (!$result) {
            wp_send_json_error(['message' => __('Error saving redirect', 'ultimakit-for-wp')]);
            return;
        }
        ...
        if (!$redirect) {
            wp_send_json_error(['message' => __('Error retrieving redirect data', 'ultimakit-for-wp')]);
            return;
        }
    ...
    public function show_redirect_notice() {
        ...
        <div class="notice notice-warning is-dismissible">
            <p>
                <?php 
                printf(
                    __('The %s "%s" was moved to trash. Would you like to <a href="%s">set up a redirect</a> for it?', 'ultimakit-for-wp'),
