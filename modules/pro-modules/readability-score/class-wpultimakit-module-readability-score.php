...
class UltimaKit_Module_Readability_Score extends UltimaKit_Module_Manager {
	...
	protected function initializeModule() {
		...
		if ( $this->is_active ) {
            ...
            add_action('add_meta_boxes', [$this, 'add_readability_meta_box']);
            add_action('save_post', [$this, 'save_readability_score']);
            add_action('wp_ajax_calculate_readability', [$this, 'ajax_calculate_readability']);
    ...
    public function enqueue_admin_assets($hook) {
        ...
        wp_localize_script($this->ID, 'wpukReadabilityScore', array(
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="history-trend <?php echo esc_attr($trend_class); ?>">
                                            <?php echo $change > 0 ? '+' : ''; echo round($change, 1); ?>
                                        </span>
                                    </td>
    ...
    private function get_score_label($score) {
        if ($score >= 90) return __('Very Easy to Read', 'ultimakit-for-wp');
        if ($score >= 80) return __('Easy to Read', 'ultimakit-for-wp');
        if ($score >= 70) return __('Fairly Easy to Read', 'ultimakit-for-wp');
        if ($score >= 60) return __('Standard', 'ultimakit-for-wp');
        if ($score >= 50) return __('Fairly Difficult', 'ultimakit-for-wp');
        if ($score >= 30) return __('Difficult', 'ultimakit-for-wp');
    ...
    public function ajax_calculate_readability() {
        ...
        if (!current_user_can('edit_posts')) {
            wp_send_json_error(['message' => __('Permission denied', 'ultimakit-for-wp')]);
            return;
        }

        $post_id = isset($_POST['post_id']) ? absint($_POST['post_id']) : 0;
        if (!$post_id) {
            wp_send_json_error(['message' => __('Invalid post ID', 'ultimakit-for-wp')]);
            return;
        }
